...
# SD 1.5
# Image_model_id = "Lykon/dreamshaper-7"
# Image_adapter_id = "latent-consistency/lcm-lora-sdv1-5"
# SDXL lora
Image_model_id = "stabilityai/stable-diffusion-xl-base-1.0"
Image_adapter_id = "latent-consistency/lcm-lora-sdxl"
# loading Image model for accelerated generation, can be commented out
# if using 1 GPU only
...
def generate_images(args):
    ...
    dataset_class_ids = [id for sublist in args.train_class_ids for id in sublist]
    ...
    if not hasattr(args, "n_train_images_per_class"):
        # get number of images per class from the number of captions if not passed
        args.n_train_images_per_class = len(args.captions) // len(dataset_class_ids)
    
    # load and fuse lcm lora
    pipe.load_lora_weights(adapter_id)
    pipe.fuse_lora()
    for index in range(len(dataset_class_ids)):
        for index2 in range(args.n_train_images_per_class):
            ...
            image = pipe(prompt_embeds=conditioning, pooled_prompt_embeds=pooled, num_inference_steps=6, guidance_scale=args.guidance_scale,requires_safety_checker=False,safety_checker=None,).images[0]

            image_path = os.path.join(
                    savedir,
                    "images",
                    f"{dataset_class_ids[index]:03d}",
                    f"{index2:03d}.png",
                )
...
def generate_images_accelerate(args):
    """Generate images based on images caption stored in args. This version uses accelerate to run on multiple GPUs."""
    # get rank
    state = PartialState()
    ...
    dataset_class_ids_local = np.array_split(dataset_class_ids, args.devices)[rank]
    dataset_class_ids_local_counter = np.array_split(np.array(range(len(dataset_class_ids))), args.devices)[rank]
    # dataset_class_ids_local = [dataset_class_ids_local[-1]]
    print("local rank:", rank)
    print("local class ids:",dataset_class_ids_local)
    for class_id in dataset_class_ids_local:
        path = os.path.join(savedir, "images", f"{class_id:03d}")
        os.makedirs(path, exist_ok=True)
    ...
    # CLIP filter (not used so commented out)
    ...
    for index in range(len(dataset_class_ids_local)):
        index2=0
        while index2<args.n_train_images_per_class:
            # tic = time.perf_counter()
            print("local rank:", rank)
            print("Generating (accelerated) image: ", index2, "for class: ", dataset_class_ids_local[index])
            # remember to shift entry by first class for current rank
            image_caption=args.captions[(dataset_class_ids_local_counter[0]+index)*args.n_train_images_per_class+index2]
            ...
            #    if (indices[0]==int(dataset_class_ids_local[index])):
            #        print("Image accepted")
            #        image_path = os.path.join(
            #           savedir,
            #            "images",
            #            f"{dataset_class_ids_local[index]:03d}",
            #            f"{index2:03d}.png",
            #        )
            #        image.save(image_path)
            #        index2=index2+1
